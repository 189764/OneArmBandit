...
public class SerializationLibrary extends AbstractResourcesLibrary {
	 ...
	 */
	@Override
	public List<Object> getObjects(String structure) throws LackOfFileException, BadStructureException {
		Object obj;
		...
		if (isRepresentsMoreThanOne(dir)) {
			list = this.createObjectsFromXMLFile(this.createSavingPathToDirs(dir.getDirs()), editStructure);

		} else {
			obj = this.createObjectFromXMLFile(this.createSavingPathToDirs(dir.getDirs()),
					this.createSavingPathToResource(dir.getContent()));
	...
	/**
	 * This method checks if structure path to saving files contains proper
	 * signs.
	 */
	@Override
	...
	/**
	 * This method checks if structure path to download files contains proper
	 * signs.
	 */
	...
	/**
	 * This method will save directories. Directories are some kind of memory of
	 * library.
	 */
	...
	/**
	 * This method will load directories from disk. It has supress warning
	 * because we have to convert object to List
	 */
	...
	/**
	 * This method creates path, predicted to saving many files. It changes dirs
	 * from List<String> into string with slashes.
	 * 
	 * @param dirs
	 *            List of dirs, which is returned by Directory object.
	...
	/**
	 * This method creates saving path, predicted to saving one file. It changes
	 * dirs and files from list into path.
	 * 
	 * @param list
	 *            - list of dirs and file.
	 ...
	 * 
	 * @param object
	 *            - obcject to change into xml file
	 * @param dirs
	 *            - path - only with directories
	 * @param newPath
	 *            - path - directories with file
	 */
	...
	private Object createObjectFromXMLFile(String dirs, String newPath) {
		Object obj;
		File file = new File(dirs);
		if (file.isDirectory()) {
			try {
	...
	/**
	 * This method creates paths to many files. It connects files from list with
	 * path named dir
	 * 
	 * @param files
	 *            - names of files, which must be connected witdh dir
	 * @param dir
	 *            - path - without files
	 ...
	 *            - path - only dirs
	 * @param structure
	 *            - adressing structure - used to equaling it with structure
	 *            returned by Directory object
