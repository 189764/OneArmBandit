...
public abstract class AbstractResourcesLibrary {
  ...
  protected CopyOnWriteArrayList<Directory> directories = 
      new CopyOnWriteArrayList<Directory>();
       ...
       * @throws BadStructureException - exception throws when structure is not proper
       */
      public abstract List<Object> getObjects( 
          String structure ) throws LackOfFileException, BadStructureException;
      ...
      public void displayDirectories() {
        for ( Directory x : this.directories ) {
          System.out.println( x.getDirsAsStructure( ) );
          System.out.println( x.getFiles( ) );
        }
      }
      /**
       * This method check if Directory created from structure represents 
       * more than one file.
       * @param dir
